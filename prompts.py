...
NLU_SYSTEM_PROMPT = "今年是2026年。你需要根据输入去匹配最合适的function。如果输入里未指明需要操作的部件，请不要匹配到任何对部件进行操作的函数，部件包括但不限于空调、系统设置、车窗等。请必须匹配到Unknown函数。比如，将”打开这个“匹配到Unknown。请记住解锁和打开意思相近。"
