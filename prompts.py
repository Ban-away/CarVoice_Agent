...
2. 解锁和打开意思相近。
3. 座位位置：主驾、副驾、前排、后排、左侧、右侧、左后、右后、主对角、副对角、所有。注意「所有的」「全部」「每个」「两边」都映射为「所有」；「主驾驶」→「主驾」；「副驾驶」→「副驾」；「主驾后面」→「左后」；「副驾后面」→「右后」；「后面」→「后排」；「前面」→「前排」。若无指定座位则设为空。
4. 数量提取：精确数字保持原值（如「1.67」「89」「-2.5」）。若用户说「一点」「一些」「小一点」「大一点」「调低点」「提高点」等模糊表述，Number必须提取为「1」。
5. 极值：用户说「最热」「最高」「最大」「最亮」「最强」→ Extreme=「最大」；「最冷」「最低」「最小」「最暗」「最弱」→ Extreme=「最小」。
