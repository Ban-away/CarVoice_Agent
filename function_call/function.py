        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "Media_Source": {
                        "type": "string",
                        "description": "若无明确的指定媒体源，则设为空。若有明确的指定媒体源，将其提取并映射为书面表达。有效值：歌曲、音乐、歌单、专辑、USB音乐、蓝牙音乐、手机音乐、本地电台、在线电台、在线广播、在线收音机、新闻、云听、栏目。"
                                       "映射规则：1.相声、小品、电视剧→在线电台；2.新闻、云听、栏目、在线收音机、USB音乐、蓝牙音乐、手机音乐保持原值不映射；3.当用户说'这首usb音乐'、'usb音乐播放列表'、'手机音乐播放列表'时，应提取为「歌曲」；4.'音乐界面'、'打开音乐'→「音乐」。"
                    }
                },
                ...
                "properties": {
                    "Media_Source": {
                        ...
                        "description": "若无明确的指定媒体源，则设为空。若有明确的指定媒体源，将其提取并映射为书面表达，比如歌曲，歌单，专辑，在线电台，在线广播等。"
                                       "相声、小品、电视剧请映射为在线电台。新闻不映射为在线电台，保持为「新闻」。USB音乐、蓝牙音乐、本地电台保持原值"
                    }
                },
                "required": ["Media_Source"]
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "Number": {
                        "type": "string",
                        "description": "提取出小数、负数、正整数，不提取百分数和分数。仅在输入包含明确数字或明确程度增量（如'一点'、'一些'、'调高点'）时才提取。纯描述性语句如'太冷了'、'暗得很'、'性能差'等不含数字也不含增量表达时，Number为空。若无则为空。注意：若输入含最字则提取Extreme而非Number"
                    },
                    "Ratio": {
                        "type": "string",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "Extreme": {
                        "type": "string",
                        "description": "用户表达极端程度时提取。如“最大/最高/最强/最亮/最热/最足”→“最大”；“最小/最低/最弱/最暗/最冷”→“最小”。仅当输入包含“最”字时才提取，否则为空。注意：当提取Extreme时不要提取Number"
                    }
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "Sound_Source": {
                        "type": "string",
                        "description": "声音来源，仅包括导航、电话、媒体、通知、语音和所有。'音响音量'、'音响'、'媒体音量'、'音乐声音'、'播放的声音'均设为「媒体」。仅当用户明确说所有音量/全部音量/静音时才设为所有。若用户只说声音/音量未指明来源，则为空"
                    },
                    "Number": {
                        "type": "string",
                        "description": "提取出小数、负数、正整数，不提取百分数和分数。仅在输入包含明确数字或明确程度增量（如'一点'、'一些'、'调高点'）时才提取。纯描述性语句如'太冷了'、'暗得很'、'性能差'等不含数字也不含增量表达时，Number为空。若无则为空。注意：若输入含最字则提取Extreme而非Number"
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "用户想要知道车辆状态是否有异常，或想要查看轮胎胎压、油耗、续航（剩余油量）、总里程数、维保计划（不包括预约维保或取消预约维保）。"
                           "如果用户只是笼统地问车辆状态、有没有问题、故障等，则所有参数都不提取。只有在用户非常明确地只关注其中一项时才提取：Tire=1（仅胎压），gas=1（仅油耗），range=1（仅续航/加油），total=1（仅总里程），Maintenance=1（仅保养）。若不确定则全部为空",
            "parameters": {
                "type": "object",
                "properties": {
                    "Tire": {
